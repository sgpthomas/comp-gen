...
I basically want to generate the above table automatically. These are the steps needed to do a single full run:

- Compile the comp-gen executable for the language
- on the server, run =./dios-lang ..= with the each config in =params/=
  - have timeout + memory limit
- Record some data (initial and final program depth, e-graph cost, saturation, phase stats)
- Copy back =kernel.c=
- Simulate with x-sim
- Report data
