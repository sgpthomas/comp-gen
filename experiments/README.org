...
compile_config="experiments/filtering/test.json"
alias r="rsync -e 'ssh -o StrictHostKeyChecking=no' -v"
# r $rule_config ubuntu@$IP:~/rules.json
# r $compile_config ubuntu@$IP:~/compile.json

cargo build --release --manifest-path dios-lang/Cargo.toml
r dios-lang/target/release/dios-lang ubuntu@$IP:
...
: dios-lang
: 
: sent 27,011,117 bytes  received 33,443 bytes  10,817,824.00 bytes/sec
: total size is 30,954,480  speedup is 1.14
...
rsync -e 'ssh -o StrictHostKeyChecking=no' -v \
      ...
      ~/Research/comp-gen/experiments/cycle-estimation/q-prod/
#+end_src
