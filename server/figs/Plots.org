...
python3 evaluation/eval_benchmarks.py --skiplargemem --skiprun -o results
#+end_src
...
TODO: generate the stock_cycles csv file.
...
data %>%
  ...
  ## ylim(0, 1.5) +
  ## scale_fill_discrete(labels=c("Compgen", "Stock Dios")) +
  labs(y="Log Normalized Compile Time", fill="Compiler") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, vjust = 0.9, hjust=1),
    axis.title.x = element_blank(),
