...
podman logs --tail 10 --latest --follow
#+end_src
...
#+begin_src R :results graphics file :file rule_distribution.svg
source("R/rule_distribution.R")
rule_distribution()
#+end_src

#+RESULTS:
[[file:rule_distribution.svg]]

#+begin_src R :results graphics file :file alpha_beta.svg
data <- read_csv("data/alpha_beta.csv")

data %>%
  ggplot(aes(x = as.factor(beta), y = as.factor(alpha), fill = cycles)) +
  geom_tile(linewidth=1000)
...
#+begin_src R :results graphics file :file play.svg
L <- 300
H <- 1500
W <- 100

## f <- function(x) x
## g <- function(x) x - H + L + W
## m <- function(x) (((g(H) - f(L)) / (H - L)) * (x - L)) + f(L)
## minv <- function(x) (((H - L) / (g(H) - f(L))) * (x - L)) + f(L)
...
tibble(x = -2020:2020) %>%
  mutate(
    ...
    ## mx = m(x),
    ## minvx = minv(x),
    px    = piecewise(L, H, W)(x),
    pxinv = piecewise(L, H, W, inv=T)(x)
  ) %>%
  ## print(n=100) %>%
  ggplot(aes(x = x)) +
  ylim(-2050, 2050) + xlim(-2050, 2050) +
  ## geom_line(aes(y = fx), color="red") +
  ## geom_line(aes(y = mx), color="blue") +
  ## geom_line(aes(y = minvx), color="blue") +
  ## geom_line(aes(y = gx), color="green") +
  geom_line(aes(y = px), color="black", size=2) +
  geom_line(aes(y = pxinv), color="blue", size=2) +
  theme_minimal()
#+end_src 
