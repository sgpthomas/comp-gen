...
rsync -avh completed/ samthomas@sgt.csres.utexas.edu:~/Research/comp-gen/server/completed
#+end_src
...
data %>%
  mutate(
    pattern=if_else(cycles < 0, T, F),
    cycles=if_else(cycles < 0, 1500, cycles),
  ...
  geom_hline(
    yintercept=1500,
    color="red"
  ) + annotate("text", x=3, y=1560, label="TIMEOUT", size=8, color="red") +
  scale_fill_manual(
...
#+begin_src async-shell :dir (sgt/dir) :results none :name test :ansi t
...
cd server/jobs/Apr12-1604-mat-mul_2x2_2x2-0
./run.sh
#+end_src
